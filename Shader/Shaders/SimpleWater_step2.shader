	...
	SubShader
	{
		pass
		{
			...
			param CalculateParam(float4 vertex)
			{
				...
				float px = vertex.x + Q * A * d.x * cos(u);
				float pz = vertex.z + Q * A * d.z * cos(u);
				float py = A * sin(u);	//波1
				...
				pa.worldPos = float4(px ,py, pz , vertex.w);	//Gestner波
				// pa.worldPos = float4(vertex.x ,py, vertex.z , vertex.w);	//波1
				pa.biNormal = float3(1,by,0);
				pa.tangent 	= float3(0,ty,1);
				// pa.normal 	= float3(-by,-ty,1);//x
				pa.normal 	= float3(-by,-ty,1);//x
